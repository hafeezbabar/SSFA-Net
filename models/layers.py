...
class SLSH:
    ...
    def sort_and_split(self, input_tensor, hash_codes, spatial_size, num_groups=5):
        ...
        sorted_tensor = torch.stack(sorted_tensors)  # [batch_size, spatial_size, channels]
        
        # Split sorted_tensor into `num_groups` equal parts
        group_size = spatial_size // num_groups
        groups = [sorted_tensor[:, i * group_size:(i + 1) * group_size, :] for i in range(num_groups)]
