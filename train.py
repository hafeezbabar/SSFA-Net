...
def _train(model, args):
    ...
    if args.resume:
        ...
        write_log('Resume from %d'%epoch)
        epoch += 1

    writer = SummaryWriter(log_dir='/gdata/hafeez/Results/experiments/SSFANet/logs')
    ...
    # Log training start
    write_log(f"Training started at {timestamp}")
    write_log(f"{'='*50}")
    ...
    for epoch_idx in range(epoch, args.num_epoch + 1):
        ...
        for iter_idx, batch_data in enumerate(dataloader):
            ...
            if (iter_idx + 1) % args.print_freq == 0:
                log_message = "Time: %7.4f Epoch: %03d Iter: %4d/%4d LR: %.10f Loss content: %7.4f Loss fft: %7.4f" % (
                    iter_timer.toc(), epoch_idx, iter_idx + 1, max_iter, scheduler.get_lr()[0], iter_pixel_adder.average(),
                    iter_fft_adder.average())
                print(log_message)  # Keep original print
                write_log(log_message)  # Also write to file
        ...
        if epoch_idx % args.save_freq == 0:
            ...
            write_log(f"Model saved: {save_name}")

        log_message = "EPOCH: %02d\nElapsed time: %4.2f Epoch Pixel Loss: %7.4f Epoch FFT Loss: %7.4f" % (
        ...
        if epoch_idx % args.valid_freq == 0:
            ...
            if val >= best_psnr:
                ...
                write_log(f"New best model saved with PSNR: {best_psnr:.2f} dB")

    save_name = os.path.join(args.model_save_dir, 'Final.pkl')
    torch.save({'model': model.state_dict()}, save_name)
    write_log(f"Training completed. Final model saved: {save_name}")